...
```
/mpdev 增加一个"夜间巡检"任务类型，夜间巡检时机器人降速、关闭语音
```
...
| 模式 | 适用场景 | 示例 |
|------|---------|------|
| **Quick (A)** | 单模块小改动 | `/mpdev 给告警表加一个备注字段` |
| **Cross-Module (B)** | 跨模块特性 | `/mpdev 增加夜间巡检任务类型` |
| **Exploration (C)** | 只调查不改代码 | `/mpdev 追踪 GAME_OVER 在 5 个模块中的流转` |
...
- **升级信号整批停下**：任一 bug 触发契约变更信号 → 弹窗询问"改用 /mpdev 或继续"
...
每次跑完 `/mpdev`、`/mpdev-fix`、`/mpdev-commit` 都会在 `.claude/mpdev-runs/` 下留下完整档案：

```
.claude/mpdev-runs/
├── INDEX.md              全部历史一览（3 张表：运行 / 修复 / 提交）
├── {run_id}/             /mpdev 每次运行的 7-10 份文档
