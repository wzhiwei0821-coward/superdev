...
1. 读取对应模板 `.claude/templates/impl-{language}.tmpl`
2. 替换占位符：
...
1. 读取 `.claude/templates/architect.tmpl`
2. 汇总所有模块信息：
...
1. 如果找到契约仓库：
   - 读取 `.claude/templates/contract-designer.tmpl`
   - 注入仓库路径、目录结构、校验脚本
...
| 识别结果 | dialect 文件 |
|---------|-------------|
| `mysql` / `mariadb` | `.claude/templates/dialects/mysql.md` |
| `postgres` / `postgresql` / `opengauss` / `gaussdb` | `.claude/templates/dialects/postgresql.md` |
| `dm7` / `dm8` / `dameng` / `达梦` / `dm.jdbc` | `.claude/templates/dialects/dameng.md` |
| `kingbase` / `kingbase8` / `人大金仓` / `com.kingbase8` | `.claude/templates/dialects/kingbase.md` |
| 其他（Oracle / SQL Server / TiDB / OceanBase 等）| 兜底 `mysql.md`，在生成的 dba.md 顶部加注释"⚠️ 检测到 {DB}，当前无专用方言文件，已用 MySQL 方言兜底，需人工补充差异" |

#### 8.3 合并占位符

1. 读 `.claude/templates/dba.tmpl`
2. 读选定 dialect 文件，提取：
...
1. 读 `.claude/templates/tester.tmpl`
2. 读 `.claude/templates/test-flavors/{flavor}.md`，提取：
...
读取当前 `.claude/commands/mpdev.md`（如存在），保留通用编排框架，更新：
