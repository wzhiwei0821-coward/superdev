...
```
存在 build.gradle 或 pom.xml     → Java Spring Boot → 读 .claude/templates/understand/references/java_springboot.md
存在 package.json 且含 "vue"      → Vue 前端         → 读 .claude/templates/understand/references/vue_frontend.md
...
**references 路径**：所有指南都在 `.claude/templates/understand/references/`（套件自带，无外部依赖）。
...
reference 文件未找到 → 提示套件安装不完整（`.claude/templates/understand/references/` 缺失），重跑 install 脚本。
