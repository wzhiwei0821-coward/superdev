...
`/mpdev` 的精简版。跳过 Architect（用户已定位问题）和 Contract-Designer（bug 修复极少改契约），直接让目标模块的 impl agent 修复，最后过**一次** code-reviewer 整批把关。
...
/mpdev-fix 不改契约仓库。建议：
  A. 整批终止，改用 /mpdev 做系统性修复（推荐）
  B. 继续批量 /mpdev-fix（风险自担：契约可能漂移，最终报告会标注）
  C. 移除这些 bug，其他的继续修"
```

- 选 A → 终止，提示："请用 `/mpdev 描述整体需求` 重新启动"
...
（不存在的组跳过；unknown 在 Step 1 已清理；**注**：不含 contracts —— 涉及契约的 bug 在 Step 2 升级信号里已被拦截到 /mpdev）
...
- 若有 cannot_fix 或契约风险：建议 `/mpdev` 做系统性处理
```
