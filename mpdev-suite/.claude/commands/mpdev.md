...
**本命令是 slash command**，用户通过 `/mpdev 需求描述` 触发。
...
在 Step 2 模式识别前执行。目的：Step 2 的关键词判断必须基于**真正的需求文本**，而不是路径字符串——当用户传入 `/mpdev /path/to/prd.md` 时，`$ARGUMENTS` 只是一个路径，Step 2 看不到"任务/巡检/前端"等关键词，无法准确分类。
